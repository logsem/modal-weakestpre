...
Lemma mwp_id_lift_atommwp_step  E Φ e1 :
  to_val e1 = None →
...
Proof.
  iIntros (??).
  by iApply (mwp_lift_atommwp_step (mwpd_id mwpD_SI) _ E (λ v n _, Φ v n) e1); simpl.
Qed.

Lemma mwp_id_lift_atommwp_det_step  E Φ e1 :
  to_val e1 = None →
  Atomic StronglyAtomic e1 →
  (∀ σ1, mwpD_SI σ1 -∗
     ∃ v2 σ2,
...
Proof.
  by iIntros (??);
    iApply (mwp_lift_atommwp_det_step (mwpd_id mwpD_SI) _ E (λ v n _, Φ v n) e1).
Qed.
...
Proof.
  by intros;
    iApply (mwp_lift_atommwp_head_step (mwpd_id mwpD_SI) _ E (λ v n _, Φ v n) e1).
Qed.
...
Proof.
  iIntros (???).
  by iApply (mwp_lift_atommwp_head_step' (mwpd_id mwpD_SI) _ E (λ v n _, Φ v n) e1).
Qed.
